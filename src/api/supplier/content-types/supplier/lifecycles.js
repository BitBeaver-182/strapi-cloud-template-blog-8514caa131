 ...
 */
function resolveExcludeDocumentId(data, excludeFromEvent) {
...
async function assertUniqueNameAndSlug(strapi, data, excludeDocumentId) {
  ...
  if (name != null && String(name).trim() !== '') {
    ...
    const conflict = rows.filter(
      (r) => !effectiveExclude || String(r.documentId) !== effectiveExclude
    );
...
async function resolveDocumentIdFromWhere(strapi, where) {
  if (!where) {
    return null;
  }
...
async function resolveNameForUid(strapi, data, where) {
...
async function ensureSlugFromName(strapi, data) {
...
module.exports = {
  ...
  async beforeUpdate(event) {
    ...
    if (Object.prototype.hasOwnProperty.call(data, SLUG_FIELD) && isSlugEmpty(data[SLUG_FIELD])) {
    ...
    } else if (Object.prototype.hasOwnProperty.call(data, SLUG_FIELD) && !isSlugEmpty(data[SLUG_FIELD])) {
      await assertUniqueNameAndSlug(strapi, data, excludeDocumentId);
    }
  },
};
